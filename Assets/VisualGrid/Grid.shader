Shader "Unlit/Grid"
{
    Properties
    {
        ...
        [HDR]_GridColour("Grid Colour", Color) = (0,0,0,0)
        _GridSize ("Grid Size", Range(0,1)) = 1
        _GridLineThickness ("Grid Line Thickness", Range(0,1))=0.5
        _Alpha ("Grid Transparancy", Range(0,1)) = 0.5
    }
    SubShader
    {
        ...
        Pass
        {
            ...
            float GridTest(float2 r)
            {
                ...
                for(float i=0.0; i<=1; i=i+_GridSize)
                {
                    for(int j=0; j<2; j++)
                    {
